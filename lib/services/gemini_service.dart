...
class GeminiService {
  ...
  String get apiKeyStatus =>
      _isApiKeyValid ? 'Valid' : 'Invalid atau tidak ditemukan';
  ...
  Future<String> generatePokemonDescription(String name, String type) async {
...
''';
  ...
  // Method umum generateContent → supaya bisa dipakai di ChatScreen
  Future<String> generateContent(List<String> messages) async {
    if (!_isApiKeyValid || _model == null) {
      return 'Error: API key tidak valid atau tidak dikonfigurasi dengan benar.';
    }

    try {
      final contentList = messages.map((msg) => Content.text(msg)).toList();
      ...
      return text?.isNotEmpty == true
          ? text!
          : 'Tidak dapat menghasilkan respon saat ini.';
    } on GenerativeAIException catch (e) {
      return _handleGeminiError(e);
    } on TimeoutException catch (_) {
      return 'Timeout: Permintaan memakan waktu terlalu lama.';
    } catch (e) {
      if (kDebugMode) debugPrint('Error in generateContent: $e');
      return 'Terjadi kesalahan teknis.';
    }
  }
  ...
  Map<String, dynamic> getDebugInfo() {
    return {
      ...
      'apiKeyPrefix':
          _apiKey.length > 4 ? _apiKey.substring(0, 4) : 'N/A',
